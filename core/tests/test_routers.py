...
class TestRouters:
    ...
    def test_profile_route_404(self, db_session):
    ...
    def test_login_route_400(self,db_session):
        response = self.client.post(
            "/api/v1/users/login",
    ...
    def test_signup_route_201(self, db_session):
        ...
        check_profile = self.client.get("/api/v1/users/profile/newuser")
        assert check_profile.status_code == 200
        assert check_profile.json()["profile"]["username"] == "newuser"
    ...
    def test_delete_user_route_204(self, user, db_session, auth_client):
        response = auth_client.delete("/api/v1/users/delete")
        assert response.status_code == 204
